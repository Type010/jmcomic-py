...
def get_useable_domain():
    ...
    return list(domain_status_dict.items())

# 修改配置文件中的domain
def update_option_domain(option: str, domains: list):
    with open(option, "r", encoding="utf-8") as file:
        data = yaml.safe_load(file)
    if "impl" in data["client"]:
        data["client"]["impl"] = "html"
